...
Future<void> checkAndDownloadSDK(String location) async {
  ...
  if (!exists) {
    await downloadFile(
        Uri.parse('https://www.dropbox.com/s/vp3amdf26rh12f8/MobileRTC?dl=1'),
        iosSDKFile);
  }
  ...
  if (!exists) {
    await downloadFile(
        Uri.parse('https://www.dropbox.com/s/tv4goe438pcfs4q/MobileRTC?dl=1'),
        iosSimulateSDKFile);
  }
  ...
  if (!exists) {
    await downloadFile(
        Uri.parse(
            'https://www.dropbox.com/s/sdhn1tovwbp3y5r/commonlib.aar?dl=1'),
  ...
  if (!exists) {
    await downloadFile(
        Uri.parse(
            'https://www.dropbox.com/s/nsr6z16f5t539y6/mobilertc.aar?dl=1'),
        androidRTCLibFile);
  }
}
